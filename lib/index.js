...
export default function slackin ({
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (!email) {
      return res
      .status(400)
      .json({ msg: 'Nenhum e-mail informado' })
    }
    ...
    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'É preciso aceitar o Código de Conduta :) ' })
    }

    invite({ token, org, email, channel: chanId }, err => {
      if (err) {
        if (err.message === `Entrando no Slack...`) {
          return res
          .status(303)
      ...
      res
      .status(200)
      .json({ msg: 'Legal! Verifique seu e-mail!' })
    })
  })
