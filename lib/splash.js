...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' editores online do total de ',
          dom('b.total', total), ' editores'
        ]
        : [dom('b.total', total), ' pessoas se registraram até agora']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      dom('button.loading', 'Receber convite')
    ),
    dom('p.signin',
      'ou ',
      dom(`a href=https://${org}.slack.com target=_top`, 'entre'),
      '›'
    ),
    !iframe && dom('footer',
      'powered by ',
