...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    dom('p',
      ...
      ' no Slack<br> Entre seu e-mail abaixo para receber seu convite'
    ),
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' editores online de ',
          dom('b.total', total), ' editores'
        ]
        : [dom('b.total', total), ' editores registrados até agora']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
    ...
    dom('p.signin',
      'ou ',
      dom(`a href=https://${org}.slack.com target=_top`, 'entre agora'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
  ...
  if (!iframe) {
    ...
    if (logo) {
      ...
      css.add('.logo.org::after', {
        ...
        'font-weight': 'bold',
        'color': '#D6D6D6',
        'font-size': '50px', // can't use rem in font shorthand IE9-10
